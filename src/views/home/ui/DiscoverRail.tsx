...
import {
  saveBlindRating,
  loadBlindRatings,
} from "@/entities/blind-rating/model/blindRatings";
  ...
  return (
    <section className="bj-section">
      <SectionHead
        ...
        icon={
          <img
            src="/assets/icons/secret-envelope.png"
            alt=""
      ...
      />
      {visible.length === 0 ? (
        <div className="bj-callout bj-callout--row">
          <img
            src="/assets/icons/secret-envelope.png"
            alt=""
            className="bj-icon-img bj-icon-img--md bj-callout__icon"
          />
          <span>
              ...
              <article key={book.id} className="bj-discover-card">
                <div className="bj-discover-card__top">
                  <div className="bj-discover-card__cover">
                    <img
                      src="/assets/icons/book-blind.png"
                      alt="블라인드 책"
                      className="bj-discover-card__cover-q"
                    />
                  </div>
                  <div className="bj-discover-card__info">
                    <h3 className="bj-discover-card__title">타이틀 한 문장</h3>
                    <p className="bj-discover-card__desc bj-clamp-4">
                      {book.aiPitch}
                    </p>
                    <div className="bj-discover-card__tags">
                      {book.tags.slice(0, 3).map((tag) => (
                        <span key={tag.text} className="bj-tag">
                          #{tag.text}
                        </span>
                      ))}
                    </div>
                    {matches[book.id] !== undefined && (
                      <p className="bj-discover-card__match">
                        매칭도 {matches[book.id]}%
                      </p>
                    )}
                  </div>
                ...
                <div className="bj-discover-card__reviews">
                  {book.hints.slice(0, 2).map((hint, i) => (
                    <div key={i} className="bj-discover-card__review">
                      <span className="bj-clamp-1">{hint}</span>
                    </div>
                  ))}
                </div>
                <div className="bj-discover-card__actions">
                  <Button
                    variant="primary"
                    className="bj-btn--pill bj-discover-card__btn-primary"
                    disabled={curious}
                  ...
                  <Button
                    variant="secondary"
                    className="bj-btn--pill bj-discover-card__btn-secondary"
